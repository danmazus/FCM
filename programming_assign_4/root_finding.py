...
def display_panda_df(log):
    df = pd.DataFrame(log, columns=["Iteration", "x_k", "Error"])
...
def secant_method(f, x0, x1, rho, max_iter, tol = 1e-6):
    ...
    while k < max_iter:
        ...
        results.append((k, x_k1, err_k))

        # Check for direct convergence
        if abs(x_k_next - x_k1) < tol:
            results.append((k+1, x_k_next, err_k_next))
...
def newton_method(f, df, x0, rho, max_iter, m = 1.0, tol = 1e-6):
    ...
    while k < max_iter:
        ...
        results.append((k, x, err_k))


        if abs(x_next) < tol or abs(x_next - x) < tol:
            results.append((k+1, x_next, err_k_next))
